...
```{r}
vcfPath <- "/hpcfs/users/a1647910/210305_chromosomeProject/2_fmr1/12_selectVariants/mergedVcf/mergedVcf.vcf.gz"
gdsPath <- "/hpcfs/users/a1647910/210305_chromosomeProject/2_fmr1/12_selectVariants/gds/mergedGds.gds"
...
plot1Col <- function(variants, genes, title) 
  ...
  scale_fill_manual(
    ...
    labels = c("A", "C", "G", "T", "Filtered", "Undetected")
  ) +
  labs(
    title = title,
    fill = "Allele"
...
```{r, results="hide"}
deChr14_variants <- lapply(deChr14, getVariants, 100)
```

```{r, fig.height=9}
plot1Col(deChr14_variants, deChr14, "DE genes on chromosome 14")
```

### DE other chrs

```{r, results="hide"}
deOther_variants <- lapply(deOther, getVariants, 100)
```

```{r, fig.height=9}
plot1Col(deOther_variants, deOther, "DE genes on other chromosomes")
```

### Not DE chr 14

```{r, results="hide"}
ndeChr14_variants <- lapply(ndeChr14, getVariants, 100)
```

```{r, fig.height=9}
plot1Col(ndeChr14_variants, ndeChr14, "DE genes on other chromosomes")
```
...
```{r, fig.height=12}
plot2Col(deChr14_variants, deChr14, "DE genes on chromosome 14")
```

### DE other chrs

```{r, fig.height=9}
plot2Col(deOther_variants, deOther, "DE genes on other chromosomes")
```

### Not DE chr 14

```{r, fig.height=9}
plot2Col(ndeChr14_variants, ndeChr14, "DE genes on other chromosomes")
```